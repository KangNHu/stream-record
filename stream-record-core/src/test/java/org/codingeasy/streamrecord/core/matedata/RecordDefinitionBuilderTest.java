...
public class RecordDefinitionBuilderTest {
	 ...
	 * @param name
	 */
	@RouteTarget(value = RecordDefinitionBuilderTest.class , paramTypes = {InterceptMethodWrapper.class})
	...
	public void getGenericLogDefinitionEl(){
		...
		//断言基本信息
		assertLogDefinitionBaseInfo(genericRecordDefinition, elMethod , ProcessorStrategy.EXPRESSION_NAME , "测试日志");
		//断言参数信息
	...
	public void getGenericLogDefinitionRouteMethod(){
		RecordDefinitionBuilder recordDefinitionBuilder = new RecordDefinitionBuilder(routeMethod, aClass);
		DefaultRecordDefinition genericLogDefinition = (DefaultRecordDefinition) recordDefinitionBuilder.getGenericRecordDefinition();
		//断言基本信息
	 ...
	 */
	private void assertLogDefinitionBaseInfo(RecordDefinition genericRecordDefinition, Method method , String processorStrategy , String temple){
		assert genericRecordDefinition.isAsync() : "异步方式解析错误";
		assert genericRecordDefinition.getRecordProducerClass().isAssignableFrom(Record.Void.class) : "日志生成器解析错误";
		assert genericRecordDefinition.getAdvice() == Advice.AFTER : "拦截方式解析错误";
		assert genericRecordDefinition.getMethod() == method : "目标方法解析错误";
		assert genericRecordDefinition.getPipelineClass().isAssignableFrom(Record.Void.class) : "管道解析错误";
		...
		assert genericRecordDefinition.getTargetClass().isAssignableFrom(RecordDefinitionBuilderTest.class) : "目标参数解析错误";
