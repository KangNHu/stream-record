package org.dream.tool.auditlog.test;
...
import org.dream.tool.auditlog.annotation.RouteTarget;
import org.dream.tool.auditlog.annotation.Search;
import org.dream.tool.auditlog.matedata.*;
import org.dream.tool.auditlog.processor.ProcessorStrategy;
import org.dream.tool.auditlog.test.model.Company;
import org.dream.tool.auditlog.test.model.User;
...
/**
*   日志构建器单元测试类
 *   @see org.dream.tool.auditlog.matedata.LogDefinitionBuilder
* @author : KangNing Hu
*/
public class LogDefinitionBuilderTest {
	...
	@AuditLog(value = "log" ,strategy = ProcessorStrategy.ROUTE_NAME)
	...
	public void getGenericLogDefinitionRouteMethod(){
		...
		assert params[0].size() == 1 : "第一个参数信息解析错误";
		//断言 路由方法信息
		assert ((Method)genericLogDefinition.get(DefaultLogDefinition.ROUTE_METHOD)).toGenericString().equals(routeTargetMethod.toGenericString()): "路由方法解析错误";
		assert Arrays.asList((Class[])genericLogDefinition.get(DefaultLogDefinition.ROUTE_METHOD_PARAM_TYPES))
				.stream().map(Class::getTypeName).collect(Collectors.joining(",")).equals("java.lang.String,org.dream.tool.auditlog.InterceptMethodWrapper"):
				"路由方法参数解析错误";
	}
	...
	private void assertLogDefinitionBaseInfo(LogDefinition genericLogDefinition  , Method method , String processorStrategy , String temple){
		...
		assert genericLogDefinition.getLogProducerClass().isAssignableFrom(AuditLog.Void.class) : "日志生成器解析错误";
		assert genericLogDefinition.getAdvice() == Advice.AFTER : "拦截方式解析错误";
		assert genericLogDefinition.getMethod() == method : "目标方法解析错误";
		assert genericLogDefinition.getPipelineClass().isAssignableFrom(AuditLog.Void.class) : "管道解析错误";
