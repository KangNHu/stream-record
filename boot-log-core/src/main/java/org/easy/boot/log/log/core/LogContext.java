...
import org.dream.tool.auditlog.core.processor.*;
import org.easy.boot.core.processor.*;
import org.easy.boot.log.core.processor.*;
import org.easy.boot.log.log.core.processor.*;
...
* @author : KangNing Hu
*/
public class LogContext implements ComponentRegistry, MethodInterceptor, ComponentFactory {
	...
	public void initComponent(){
		...
		if (this.executor == null){
			this.executor = new ThreadPoolExecutor(4 , 18 ,
					...
					new BasicThreadFactory.Builder().namingPattern("auditlog-log-").build());
		}
		this.logPostProcessors = new ArrayList<>();
	...
	public void  addLogAuditPostProcessor(LogPostProcessor logPostProcessor){
		this.logPostProcessors.add(logPostProcessor);
	}
	/**
	 * 设置模板解析器
	...
	private void applyPreProcessors(CurrentContext currentContext) {
		for (LogPostProcessor postProcessor : logPostProcessors){
