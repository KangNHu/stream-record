package org.dream.tool.auditlog.log.examples.expression;

import com.alibaba.fastjson.JSONObject;
import org.dream.tool.auditlog.configuration.EnableAuditLog;
import org.dream.tool.auditlog.core.CurrentContext;
import org.dream.tool.auditlog.core.DefaultLogProducer;
...
import org.dream.tool.auditlog.log.examples.service.impl.UserServiceImpl;
	...
	public Pipeline pipeline(){
		return auditLogInfoWrapper -> {
			...
			if (auditLogInfoWrapper.isMultiple()){
				System.out.printf("管道中已有多个结果:%s" , JSONObject.toJSONString(auditLogInfoWrapper.getAuditLogInfos()));
			}
			//如果只有一个结果
			else {
				System.out.printf("管道中已有多个结果:%s" , JSONObject.toJSONString(auditLogInfoWrapper.getAuditLogInfo()));
			}
		};
	}
