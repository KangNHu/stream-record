...
import org.dream.tool.auditlog.core.annotation.RouteTarget;
import org.dream.tool.auditlog.core.annotation.Search;
import org.dream.tool.auditlog.core.matedata.*;
...
public class LogDefinitionBuilderTest {
	...
	public void getGenericLogDefinitionRouteMethod(){
		...
		assert Arrays.asList((Class[])genericLogDefinition.get(DefaultLogDefinition.ROUTE_METHOD_PARAM_TYPES))
				.stream().map(Class::getTypeName).collect(Collectors.joining(",")).equals("java.lang.String,org.dream.tool.auditlog.core.InterceptMethodWrapper"):
				"路由方法参数解析错误";
	}
